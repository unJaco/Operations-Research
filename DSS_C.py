...
# Identifikation der Polyester Materialien
polyester_materials = [matName for matName in materialMap if "Polyester" in matName]

## Rücksendekosten ##

returnCosts = sum(remainingMaterial[matName] * variables_df.loc[0, 'Rücksendekosten'] for matName in remainingMaterial if matName not in polyester_materials)   


## Rückerstattungspreis

returnMoney = sum(remainingMaterial[matName] * materialMap[matName].costs for matName in remainingMaterial if matName not in polyester_materials)
